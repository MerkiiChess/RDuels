...
public record KitSlotsPageResolver(CustomKitStorage customKitStorage) implements PageResolver<SlotModel> {
    ...
    public List<SlotModel> resolve(Player player, InventoryContext context) {
        ...
        for (String row : settings.structure()) {
            for (char ch : row.toCharArray()) {
                ...
                String defaultMat = node.node("default-material").getString();
                if (defaultMat == null) {
                    continue;
                }
