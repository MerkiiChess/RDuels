...
public record RequestFightClickHandler(CustomKitAPI customKitAPI, DuelAPI duelAPI,
                                       ArenaAPI arenaAPI, MessageConfig config) implements ClickHandlerRegistry.ClickHandlerFacade {
    ...
    public void handle(InventoryContext context, Player player, AbstractClickHandler handler) {
        ...
        if (duelRequest.getKitModel() == null) {
            ...
            if (kitModel == null) {
                config.sendTo(player, "duel-request-kits-not-found");
                return;
            }
